...
describe('Service /datapoints and /datapoints/lookup using built config', function () {
  ...
  const configBuilt = [{
    ends_before: '2017-06-02T00:00:00.000Z',
    params: {
      query: {
        ds_id: 1
      }
    },
    path: '/datavalues-ts2008-built'
  }, {
    begins_at: '2017-06-02T00:00:00.000Z',
    params: {
      query: {
        ds_id: 2
      }
  ...
  main.app.use('/datavalues-ts2008-built', memory({
    paginate: {
      default: 200,
      max: 2000
    },
  ...
  main.app.service('/datavalues-ts2008-built').before({
    find: [
      (hook) => {
        /*
  ...
  before(function () {
    if (databases.mongodb && databases.mongodb.metadata) {
      return Promise.resolve(databases.mongodb.metadata.db).then(db => {
        ...
        return sysAdmin.service('/datastreams').patch(_idDatastream, {datapoints_config_built: configBuilt})
      }).then(() => {
        ...
        return sysAdmin.service('/datastreams').patch(_idDatastreamPref, {datapoints_config_built: configBuilt})
      })
    }
  })
