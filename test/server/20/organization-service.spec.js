...
const dataFile = 'erczo.organization'
...
describe(`Service ${servicePath}`, function () {
  ...
  const cleanup = async () => {
    await coll.organizations.remove()
  }
      ...
      return helper
        ...
        .then(({ retDoc }) => {
          expect(retDoc).to.have.property(
            'name',
            'Eel River Critical Zone Observatory - Patched'
          )
        })
    })
  })
    ...
    it('sys admin should update without error', function () {
      return helper
        ...
        .then(({ retDoc }) => {
          expect(retDoc).to.have.property(
            'name',
            'Eel River Critical Zone Observatory - Updated'
          )
        })
    })
  })
