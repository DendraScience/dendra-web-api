...
describe('Service /datastreams/lookup', function () {
  ...
  describe('#find()', function () {
    ...
    it('should not find using _id, passing non-matching identifier string', function () {
      return guest.service('/datastreams/lookup').find({query: {
        _id: 'ABC'
      }}).then(res => {
    ...
    it('should find using source and imperial som_id', function () {
      return guest.service('/datastreams/lookup').find({query: {
        ...
        expect(res).to.have.lengthOf(5).and.satisfy(datastreams => {
          return datastreams.find(ds => ds._id === _id6a) &&
            datastreams.find(ds => ds._id === _id6b) &&
            datastreams.find(ds => ds._id === _id6c) &&
            datastreams.find(ds => ds._id === _id7aFoot) &&
            datastreams.find(ds => ds._id === _id7bFoot)
        })
      })
    })
      ...
      return guest.service('/datastreams/lookup').find({query: {
        ...
        expect(res).to.have.lengthOf(5).and.satisfy(datastreams => {
          return datastreams.find(ds => ds._id === _id6a) &&
            datastreams.find(ds => ds._id === _id6b) &&
            datastreams.find(ds => ds._id === _id6c) &&
            datastreams.find(ds => ds._id === _id7aMeter) &&
            datastreams.find(ds => ds._id === _id7bMeter)
        })
      })
    })
  })
