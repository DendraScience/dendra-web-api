...
class Service {
  ...
  async find(params) {
    ...
    if (!cfg) throw new errors.GeneralError('Influx flux database config undefined.');
    if (!(fn === undefined || fn === 'count')) throw new errors.GeneralError(`Influx flux fn '${fn}' not supported.`);
    ...
    const queryStartDate = isTimeGt ? new Date(time.$gt.getTime() + 1) : isTimeGte ? time.$gte : new Date(MIN_TIME);
    const queryStopDate = isTimeLt ? time.$lt : isTimeLte ? new Date(time.$lte.getTime() + 1) : new Date(MAX_TIME);
    let q;
    if (typeof custom === 'object' && typeof custom[queryKey] === 'string') {
      ...
      q = custom[queryKey].replace(/@{([.\w]+)}/g, (_, v) => toFluxValue(vars[v]));
    } else {
      if (typeof bucket !== 'string') throw new errors.GeneralError('Influx flux requries query.bucket (string).');

      // Use from() to define the bucket
      const parts = [`from(bucket: ${toFluxValue(bucket)})`];

      // Use range() to limit query results by time
      parts.push(`range(start: ${toFluxValue(queryStartDate)}, stop: ${toFluxValue(queryStopDate)})`);

      // Use filter() to identify the measurement
      parts.push(`filter(fn: (r) => r._measurement == ${toFluxValue(typeof measurement === 'string' ? measurement : 'logger_data')})`);

      // Use filter() to narrow down to specific tag key-value pairs
      if (typeof tagSet === 'object' && Object.keys(tagSet).length) parts.push('filter(fn: (r) => ' + Object.entries(tagSet).map(([k, v]) => `r[${toFluxValue(k + '')}] == ${toFluxValue(v + '')}`).join(' and ') + ')');

      // Use filter() to select specific fields
      if (Array.isArray(fields) && fields.length) parts.push('filter(fn: (r) => ' + fields.map(name => `r._field == ${toFluxValue(name)}`).join(' or ') + ')');

      // Functions require special handling
      if (fn) {
        parts.push(fn + '()');
        parts.push('pivot(rowKey: ["_measurement"], columnKey: ["_field"], valueColumn: "_value")');
        ...
        parts.push(`drop(columns: ${toFluxValue(['_measurement', '_start', '_stop', ...(typeof tagSet === 'object' ? Object.keys(tagSet) : [])])})`);
      }
      q = parts.join('\n |> ');
    }
...
module.exports = function (app) {
  ...
  app.use('/influx/flux', new Service({
