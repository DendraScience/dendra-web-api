...
module.exports = (stages = []) => {
  return async context => {
    if (context.type !== 'before') {
      throw new Error(
        "The 'accessFind' hook should only be used as a 'before' hook."
      )
    }
    ...
    if (accessQuery && Object.keys(accessQuery).length)
    ...
    if (filters.$limit === 0) {
      ...
      if (filters.$sort) {
        data.push({ $sort: filters.$sort })
      } else {
        data.push({ $sort: { _id: 1 } })
      }
      if (filters.$skip) {
        data.push({ $skip: filters.$skip })
      }
      if (filters.$limit) {
        data.push({ $limit: filters.$limit })
      }
