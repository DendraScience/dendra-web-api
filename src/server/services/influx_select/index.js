...
class Service {
  ...
  async find(params) {
    ...
    const { api, db, sc, fc, wc, gbc, $sort: sort, $limit: limit } = query
    const apiConfig = this.apis[api] || this.apis.default
    const influxUrl = apiConfig && apiConfig.url

    if (!influxUrl)
      throw new errors.GeneralError('Influx select URL undefined.')
    if (!db) throw new errors.GeneralError('Influx select requries query.db.')

    const parts = [
      `SELECT ${typeof sc === 'string' ? sc : '*'}`,
      `FROM ${typeof fc === 'string' ? fc : 'logger_data'}`
    ]
